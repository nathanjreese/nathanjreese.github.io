<template>
  <div class="wordle-main">
    <title-page
      title-text="IndyCar Wordle"
      >
      </title-page>
    <div class="wordle-title">	
    </div>
      <word-row class="wordrow"
...
'yeley'
  ...
  methods: {
    ...
    onKeyPress(button) {
      if( !this.wonGame && !this.lostGame){
        if (this.input.length > 4 && button === 'enter'){
          if(this.words.includes(this.input) || this.allowed.includes(this.input)){
          
            if (this.state.currentGuessIndex === 0){
              this.state.solution = this.words[(Math.floor(Math.random() * this.words.length))]
            }
            let inputLetters = []
            for (let i = 0; i < 5; i++){
              let c = this.input.charAt(i)
              if (this.state.solution.indexOf(c) === -1) {
                this.state.guessedLetters.miss.push(c)
              }
              else if (c === this.state.solution.charAt(i)){
                this.state.guessedLetters.found.push(c)
              }
              else {
                if(inputLetters.includes(c)) {
                }
                this.state.guessedLetters.hint.push(c)
              }
        ...
        else if (this.input.length <= 4 && button === 'enter'){
